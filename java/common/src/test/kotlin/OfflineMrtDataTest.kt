...
class OfflineMrtDataTest {
    @Test
    fun testOfflineMrtData() {
        println("🚀 OFFLINE VERİ TESTİ: Hals-Weichteile MRT - Berlin")
        println("====================================================")
        
        val jsonFile = File("src/main/resources/berlin_mrt_doctors.json")
        if (!jsonFile.exists()) {
            println("❌ Dosya bulunamadı: ${jsonFile.absolutePath}")
            return
        }
